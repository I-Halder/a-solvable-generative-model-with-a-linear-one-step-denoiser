...
class DiffusionModel(pl.LightningModule):
    def __init__(self, in_size, t_range, img_depth):
        ...
        self.px=int(numpy.sqrt(in_size))
        ...
        self.sa1 = SAWrapper(256, int(self.px/4))
        self.sa2 = SAWrapper(256, int(self.px/8))
    ...
    def forward(self, x, t):
        ...
        x3 = self.down2(x2) + self.pos_encoding(t, 256, int(px/4))
        x3 = self.sa1(x3)
        x4 = self.down3(x3) + self.pos_encoding(t, 256,int(px/8) )
        x4 = self.sa2(x4)
        x = self.up1(x4, x3) + self.pos_encoding(t, 128, int(px/4))
        x = self.sa3(x)
        x = self.up2(x, x2) + self.pos_encoding(t, 64, int(px/2))
        x = self.up3(x, x1) + self.pos_encoding(t, 64, int(px))
        output = self.outc(x)
        return output
